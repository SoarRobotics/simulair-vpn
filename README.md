...
This sections shows my ip in the VPN network. VPN Server gets the 10.8.0.1 IP on this configuration.
## Testing the UDP Multicast Connectivity 
Ros2 uses UDP ports for making topic search inside the local network. Our VPN server must support UDP Multicasting for this kind of operation. We will use netcat app.
Open OpenVPN Server SSH prompt and command prompt of the clent device side by side.
On Server type:

    nc -lu -p 3125
On Client type 

    nc -vzu 10.8.0.1 3125
You can really use any other number, but use the same port for both server and client.

If UDP multicast is supported, you should see "X" letters are printed in a row. If this happens, you are good to go. Just set ROS_DOMAIN_IP environment variable to same number in both devices, and your Ros2 environments can make searchs on eachother.

# 2-Connect to Experiment Server
Clone this repo:
    git clone https://github.com/soar-robotics/Simulair-VPN.git
    cd Simulair-VPN/experiment-server
...
#### Connect to experiment server
    ...
    openvpn3 config-import --config cagatay.ovpn
    openvpn3 session-start --config cagatay.ovpn
# 3- SSH into experiment server
Clone this repo:
    git clone https://github.com/soar-robotics/Simulair-VPN.git
    cd Simulair-VPN/experiment-server

Type the following command for establishing ssh connection into your instance. 

    ssh -i Server.pem ubuntu@3.126.4.36
 if you get an error like
